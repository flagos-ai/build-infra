...
# Vendor build-time patches (wheels/<vendor>/patches/*.patch). Copied into the
# work dir and applied on the host below — never inside the build container.
if [[ -d "$SCRIPT_DIR/wheels/${VENDOR}/patches" ]]; then
    cp -r "$SCRIPT_DIR/wheels/${VENDOR}/patches" "$WORK_DIR/patches"
fi

# ── Pull + patch source (host-side) ──────────────────────────────────────
#
# The source tarball is downloaded, extracted and patched on the host, never
# inside the build container: the container consumes an already-patched tree,
# so `patch` is not a build-image dependency and the applied changes stay
# where the build is audited (user constraint: no patching inside containers).
...
# Vendor build-time patches (wheels/<vendor>/patches/*.patch), if any. GNU
# patch ignores the license-header preamble before the first `---` line;
# hunks are generated by `diff -u` from the modified source (metax ADR §5.5:
# JIT/cudnn/fp8 fallbacks), so a hunk failing here means the source tarball
# drifted — fail loudly rather than ship an unpatched wheel.
if [[ -d "${WORK_DIR}/patches" ]]; then
    if ! command -v patch >/dev/null 2>&1; then
        echo 'ERROR: patch(1) not found on host' >&2
        exit 1
    fi
    for p in "${WORK_DIR}"/patches/*.patch; do
        echo "==> Applying $(basename "${p}") …"
        patch -d "${WORK_DIR}/src/sglang" -p1 < "${p}"
    done
fi
...
# (sglang ships no pip sdist). The tarball was already pulled and patched on
# the host (above); in the container only the non-CUDA pyproject variant
