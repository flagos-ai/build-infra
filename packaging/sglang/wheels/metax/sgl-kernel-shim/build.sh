...
# Pure Python with no vendor toolchain, so this is a plain script (same shape
# as packaging/flaggems/build.sh): an isolated venv does the pip work, so the
# caller's interpreter state never matters.
set -euo pipefail
