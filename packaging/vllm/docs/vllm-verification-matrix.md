...
| 沐曦 | MACA 3.7.2.1 | vllm0.24.0 | https://github.com/flagos-ai/vllm-plugin-FL/pull/377 | OPEN |
| 沐曦 | MACA 3.8.1.3 | vllm0.24.0 | https://github.com/flagos-ai/vllm-plugin-FL/pull/377 | OPEN |
...
| NEUWARE 4.4.3 | ✅ | ✅ | ✅ | — |
| NEUWARE 4.7.2 | ✅ | ✅ | ✅ | — |
| TOPS 1.9.10 | ✅ | ⬜ | ⬜ | app 镜像已发布但验证记录已撤销（2026-09-03）：record verify 跑在默认 flagtree 编译器，未覆盖 vendor triton 交付路径；1.9.10 triton 路径 engine init 崩。基于完整 PR 链条（plugin #432 + flag_gems master）重做后再记录。 |
| TOPS 1.10.6 | ✅ | ✅ | ✅ | app 镜像已发布但验证记录已撤销（2026-09-03）：record verify 跑在默认 flagtree 编译器，未覆盖 vendor triton 交付路径；triton 路径未验。基于完整 PR 链条（plugin #432 + flag_gems master）重做后再记录。 |
