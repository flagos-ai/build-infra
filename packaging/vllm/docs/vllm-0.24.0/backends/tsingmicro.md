...
[VPF #421](https://github.com/flagos-ai/vllm-plugin-FL/pull/421)）。TX8110 上
...
1. **KV 缓存永不写入（跨平台根因）** —— `forward_includes_kv_cache_update`
   ...
   无覆盖 → 乱码。修复见 [VPF #421](https://github.com/flagos-ai/vllm-plugin-FL/pull/421)。
...
- vllm `0.24.0+flagos`（cp310 empty wheel，单步安装）；vllm-plugin-fl
  `0.3.0rc0+gbd010ce.d20260831`（[VPF #421](https://github.com/flagos-ai/vllm-plugin-FL/pull/421) head 构建；tsingmicro PyPI 上更早的
